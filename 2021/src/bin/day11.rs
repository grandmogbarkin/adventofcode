...
impl SolutionT for Solution {
  ...
  fn test1_result(&self) -> i64 { 1656 }
  fn test2_result(&self) -> i64 { 195 }
  ...
  fn task_1(&self, filename: String) -> Result<i64, Box<dyn Error>> {
    ...
    for _ in 0..100 {
      octopuses.iter_mut().for_each(|r| r.iter_mut().for_each(|e| *e += 1));
      for i in 0..octopuses.len() {
        for j in 0..octopuses[i].len() {
          flashes += flash(&mut octopuses, i, j);
        }
      }
      octopuses.iter_mut().for_each(|r| r.iter_mut().for_each(|e| if *e > 9 { *e = 0 }));
    }
