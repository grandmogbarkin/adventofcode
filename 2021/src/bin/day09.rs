...
impl SolutionT for Solution {
  ...
  fn test1_result(&self) -> i64 { 15 }
  fn test2_result(&self) -> i64 { 1134 }
      ...
      for j in 0..heightmap[i].len() {
        // if is_low_point(&heightmap, i, j) {
          ...
          if basin_size.len() < 3 {
            ...
            if min_basin >= cur_basin_size { continue; }
...
fn build_heightmap(input: &Vec<String>) -> Vec<Vec<i64>> {
  ...
  for line in input.iter() {
    heightmap.push(line.chars().map(|s| s.to_digit(RADIX).unwrap() as i64).collect());
  }
  heightmap
}
...
fn is_low_point(heightmap: &Vec<Vec<i64>>, i: usize, j: usize) -> bool {
  ...
  if i > 0                      { min_point = min(min_point, heightmap[i-1][j]); }
  if i < heightmap.len() - 1    { min_point = min(min_point, heightmap[i+1][j]); }
  if j > 0                      { min_point = min(min_point, heightmap[i][j-1]); }
  if j < heightmap[i].len() - 1 { min_point = min(min_point, heightmap[i][j+1]); }
...
fn basin_count(heightmap: &mut Vec<Vec<i64>>, i: usize, j: usize) -> i64 {
  ...
  if i >= heightmap.len() { return 0; }
  if j >= heightmap[i].len() { return 0; }
  if heightmap[i][j] == 9 { return 0; }
  // println!("Counting basin at {} {}", i, j);
  heightmap[i][j] = 9;
  1 + if i > 0 { basin_count(heightmap, i-1, j) } else { 0 }
    + if j > 0 { basin_count(heightmap, i, j-1) } else { 0 }
